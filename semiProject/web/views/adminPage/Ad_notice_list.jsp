...
<hr>
					...
					<div>
						...
						<button type="button" class="btn"
							style="background: black; color: white;" onclick="goWrite();">글쓰기</button>
						<button type="button" class="btn"
							style="background: black; color: white;" onclick="deleteNotice();">삭 제</button>
						
						
						<!--상단 검색창-->
						<form
							class="d-none d-md-inline-block form-inline float-right ml-auto mr-0 mr-md-3 my-2 my-md-0" method='post' action="<%=request.getContextPath() %>/search.notice">

							<div class="input-group">

								<select class="form-control" name="search">
									<option value="stitle" selected>제목</option>
									<option value="not" >일반 공지</option>
									<option value="pol">개인정보 처리 지침</option>
								...
								<!-- 검색버튼 -->
								<button class="btn btn-primary" type="submit" style="background:black; border:black;">
						...
						<table class="table table-striped table-bordered table-hover"
							...
							<tbody>
								...
								<tr>
									...
									%>
									<td id="category" class="text-center notice_list"
										style="width: 15%">일반 공지</td>

									<%
										} else if (((Board) list.get(i)).getBoard_code() == 50) {
									%>
									<td id="category" class="text-center notice_list"
										style="width: 15%">자주묻는 질문</td>

									<%
										} else {
									%>
									<td id="category" class="text-center notice_list"
										style="width: 15%">개인정보 처리 지침</td>
									<%
										}
									%>

									<td id="title" class="notice_list"><%=((Board) list.get(i)).getTitle()%></td>
									<td id="date" class="text-center notice_list"
										style="width: 20%"><%=((Board) list.get(i)).getWrite_date()%></td>
...
<script>
  		...
  		//체크된 항목 삭제
				function deleteNotice(){
            		...
            		if(confirm("선택된 항목을 삭제하시겠습니까?")){
            			...
            			var arr=new Array();
    						...
    						location.href="<%=request.getContextPath()%>/delete.notice?arr="+ arr;
            	...
            	function goWrite(){
            		...
            		location.href="<%=request.getContextPath()%>/views/adminPage/Ad_notice_insertForm.jsp";
        	...
        	//게시물 상세페이지 가기
				...
				$(function(){
					...
					$(".notice_list").click(function(){
						...
						location.href="<%=request.getContextPath()%>/detailAd.notice?board_no="+ board_no;
						});
